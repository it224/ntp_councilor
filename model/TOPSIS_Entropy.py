...
ntp_platform_TOPSIS        = db["ntp_platform_entropy_TOPSIS"]
...
def computeDistanceAnd(attributes, bestSolution, worestSolution):
    ...
    for i in range(0, 638, 1): #0~637
        ...
        pbc_dis_b_dura = ((pbc[i]["v"] - pbc_bw)**2)**0.5 #一個 attr與最佳的距離
        pnc_dis_b_dura = ((pnc[i]["v"] - pnc_bw)**2)**0.5
        npn_dis_b_dura = ((npn[i]["v"] - npn_bw)**2)**0.5
        jbs_dis_b_dura = ((jbs[i]["v"] - jbs_bw)**2)**0.5
        plat_bdis = pbc_dis_b_dura + pnc_dis_b_dura + npn_dis_b_dura + jbs_dis_b_dura

        pbc_dis_w_dura = ((pbc[i]["v"] - pbc_ww)**2)**0.5 #一個 attr與最佳的距離
        pnc_dis_w_dura = ((pnc[i]["v"] - pnc_ww)**2)**0.5
        npn_dis_w_dura = ((npn[i]["v"] - npn_ww)**2)**0.5
        jbs_dis_w_dura = ((jbs[i]["v"] - jbs_ww)**2)**0.5
